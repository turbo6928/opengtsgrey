...
$Command .= " -conf=$GTS_CONF -log.name=$SERVER_NAME";
...
if (defined $opt_kill) {
    if (-f "$PID_FILE") {
        ...
        if ($pid ne "") {
            print "Killing '$SERVER_NAME' PID: $pid\n";
...
if (defined $opt_port) {
    $DCSCommand .= " -${opt_server}.port=$opt_port";
}
if (defined $opt_cmdport) {
    $DCSCommand .= " -${opt_server}.commandPort=$opt_cmdport";
}
...
if (-f "$PID_FILE") {
    ...
    if ($cmd_ps eq "") {
        print "The '${SERVER_NAME}' server may already be running.\n";
    ...
    if ($rtn == 0) {
        print "The '${SERVER_NAME}' server is likely already running using pid $pid.\n";

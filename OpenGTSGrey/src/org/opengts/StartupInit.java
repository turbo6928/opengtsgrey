    ...
    **/
    public void preInitialization()
    {
        if (RTConfig.isWebApp()) {
            ...
            long maxMemK = rt.maxMemory()   / 1024L;
            long maxTotK = rt.totalMemory() / 1024L;
            long maxFreK = rt.freeMemory()  / 1024L;
            Print.logInfo("Memory(K): max=%d, total=%d, free=%d", maxMemK, maxTotK, maxFreK);
        }
    }
                ...
                if (isFleet) {
                    ...
                    if (optDevFields != null) {
                        ...
                        if ((ndx < optDevFields.length) && (edp instanceof EventData)) {
                            ...
                            if (device != null) {
                                return StringTools.trim(optDevFields[ndx].getFieldValue(device));
                            }
                        }
                    } else {
                    ...
                    if (optEvtFields != null) {
                        ...
                        if ((ndx < optEvtFields.length) && (edp instanceof EventData)) {
                            return StringTools.trim(optEvtFields[ndx].getFieldValue((EventData)edp));
                        }
                    } else {
                        // return custom field value
                        switch (ndx) {
    ...
    **/
    public java.util.List<DBField> selectFields(DBFactory factory, java.util.List<DBField> fields)
    {
        ...
        if (tblName.equalsIgnoreCase(Device.TABLE_NAME())) {
            ...
            addDBFields(tblName, fields, "startupInit.Device.BorderCrossingFieldInfo", false, Device.BorderCrossingFieldInfo);

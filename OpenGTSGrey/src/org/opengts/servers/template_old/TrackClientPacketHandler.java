    ...
    extends AbstractClientPacketHandler
{
    ...
    private void _insertEventData(Device device, long fixtime, 
        int statusCode, 
        GeoPoint geoPoint,
    ...
    private boolean parseInsertRecord_ASCII_1(String s)
    {
        ...
        if (SIMEVENT_GEOZONES && geoPoint.isValid()) {
            Device.GeozoneTransition zone[] = device.getGeozoneTransition(geoPoint);
            if (zone != null) {
                for (Device.GeozoneTransition z : zone) {
                    int    zoneCode = z.getStatusCode();
                    String zoneID   = z.getGeozoneID();
                    String zoneDesc = z.getGeozoneDescription();
                    long   zoneTime = z.adjustFixtime(fixtime);
                    this._insertEventData(device, zoneTime, zoneCode, geoPoint, speedKPH, heading, altitudeM, odomKM);
                    Print.logInfo(StatusCodes.GetDescription(zoneCode,null) + "    : " + zoneDesc);
                    eventCount++;
                }
            }
        }
        ...
        /* create/insert event */
        this._insertEventData(device, fixtime, statusCode, geoPoint, speedKPH, heading, altitudeM, odomKM);
        eventCount++;

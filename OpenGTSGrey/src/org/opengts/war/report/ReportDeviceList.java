    ...
    extends DBRecord<ReportDeviceList> // not really a database table
{
    ...
    {
        if (this.devMap == null) {
            this.devMap = new HashMap<String,DeviceHolder>(4);
        }
        return this.devMap;
    }
    ...
    public boolean add(DeviceGroup group)
    {
        ...
        User    user = this.getUser();
        try {
            OrderedSet<String> devIDSet = DeviceGroup.getDeviceIDsForGroup(acctID, group.getGroupID(), null/*User*/);
    ...
    public void addAllAuthorizedDevices()
    {
        try {
            ...
            Account acct = this.getAccount();
            OrderedSet<String> list = User.getAuthorizedDeviceIDs(usr, acct);
    ...
    public void clear()
    {
        this.getDeviceMap().clear(); // set Map to empty
    }

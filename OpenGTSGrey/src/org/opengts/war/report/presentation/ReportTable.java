    ...
    implements ReportPresentation
{
        ...
        throws ReportException
    {
        ...
        switch (format) {
            ...
            case CSV :
                return this.writeCSV( out, indentLevel, rd);
            case HTML:
            default:
        ...
        throws ReportException
    {
        ...
        /* report subtitle row */
        {
            ...
            // "Refresh"
            out.print("<td>");
            String refreshURL = !isEMail? EncodeURL(reqState, rd.getRefreshURL()) : null;
            if (!StringTools.isBlank(refreshURL)) {
                String refreshDesc = i18n.getString("ReportTable.refreshReport","Refresh");
                out.print("<a href='"+refreshURL+"' target='_top'>"+refreshDesc+"</a>");
            } else {
                out.print("&nbsp;");
            }
            out.print("</td>\n");
            ...
            String graphURL = !isEMail && rd.getSupportsGraphDisplay()? EncodeURL(reqState,rd.getGraphURL()) : null;
            if (!StringTools.isBlank(graphURL)) {
                ...
                if (linkSB.length() > 0) { linkSB.append("&nbsp;&nbsp;"); } // add space between links
                linkSB.append("<span class='spanLink' onclick=\"javascript:openResizableWindow('"+graphURL+"','ReportGraph',"+sz.getWidth()+","+sz.getHeight()+");\">"+desc+"</span>");
            }
            String mapURL = !isEMail && rd.getSupportsMapDisplay()? EncodeURL(reqState,rd.getMapURL()) : null;
            if (!StringTools.isBlank(mapURL)) {
                ...
                if (linkSB.length() > 0) { linkSB.append("&nbsp;&nbsp;"); } // add space between links
                linkSB.append("<span class='spanLink' onclick=\"javascript:openResizableWindow('"+mapURL+"','ReportMap',"+sz.getWidth()+","+sz.getHeight()+");\">"+desc+"</span>");
            }
            String kmlURL = !isEMail && rd.getSupportsKmlDisplay() && privLabel.getBooleanProperty(PrivateLabel.PROP_ReportDisplay_showGoogleKML,false)? EncodeURL(reqState,rd.getKmlURL()) : null;
            if (!StringTools.isBlank(kmlURL)) {
                ...
                if (linkSB.length() > 0) { linkSB.append("&nbsp;&nbsp;"); } // add space between links
                linkSB.append("<a href='"+kmlURL+"' target='_blank'>"+desc+"</a>");
            }
            if (linkSB.length() > 0) {
                out.print(linkSB.toString());
            } else {
                out.print("&nbsp;");
            }
            out.print("</td>\n");
    ...
    private int writeCSV(PrintWriter out, int level, ReportData rd) 
        throws ReportException
    {

    ...
    implements MapProvider
{
    ...
    **/
    ...
    public String _getIconLegendHtml(String legendXml, RequestProperties reqState, boolean outputHtml)
    {
        ...
        if (!xml.startsWith("<"+TAG_Legend+">")) {
            Print.logError("Legend XML does not start with <"+TAG_Legend+">: \n" + xml);
            return "";
        }
        ...
        if (xmlDoc == null) {
            Print.logError("Invalid Legend XML: \n" + xml);
        ...
        return this._getIconLegendHtml(legend, reqState, outputHtml);
        
    }
    
    public String _getIconLegendHtml(Element legend, RequestProperties reqState, boolean outputHtml)
    {
        ...
        /* create html table */
        NodeList titleList = legend.getElementsByTagName(TAG_Title);
        if (titleList.getLength() > 0) {
            Element titleElem = (Element)titleList.item(0);
            String  title = RTConfig.insertKeyValues(XMLTools.getNodeText(titleElem," ",false).trim());
            if (outputHtml) {
                sb.append("<span class=\""+CSS_legend+"\">"+title+"</span>\n");
            } else {
                sb.append("<"+TAG_Title+"><![CDATA["+title+"]]></"+TAG_Title+">\n");
            }
        } else
        if (outputHtml && (reqState != null)) {
            PrivateLabel privLabel = reqState.getPrivateLabel();
            I18N   i18n  = privLabel.getI18N(MapProviderAdapter.class);
            String title = i18n.getString("MapProviderAdapter.legendTitle","Pushpin Legend");
            sb.append("<span class=\""+CSS_legend+"\">" + title + "</span>\n");
        }
        if (outputHtml) {
        ...
        NodeList iconList = legend.getElementsByTagName(TAG_Icon);
        for (int i = 0; i < iconList.getLength(); i++) {
            Element icon  = (Element)iconList.item(i);
            String  name  = XMLTools.getAttribute(icon,ATTR_name,null,false);
            double  scale = StringTools.parseDouble(XMLTools.getAttribute(icon,ATTR_scale,null,false),0.0);
            String  desc  = RTConfig.insertKeyValues(XMLTools.getNodeText(icon,"",false).trim());
            PushpinIcon pp = pushpinMap.get(name);
            if (pp == null) {
                Print.logWarn("PushpinIcon not found: " + name);
                continue;
            }
            String imgUrl = pp.getIconURL();
            if (StringTools.isBlank(imgUrl)) {
                Print.logWarn("Invalid PushpinIcon URL: " + imgUrl);
                continue;
            }
            if (outputHtml) {
    ...
    *** @param isFleet  True if for fleet maps
    *** @return The maximum number of allowed pushpins
    **/
    public long getMaxPushpins(boolean isFleet)
    {
        RTProperties rtp = this.getProperties();
        String propName[] = isFleet? 
    ...
    **/
    protected String getMapCellStyle(RequestProperties reqState, MapDimension mapDim)
    {
        ...
        String styleW = (mapW > 0)? (""+mapW+"px") : "99%";
        String styleH = (mapH > 0)? (""+mapH+"px") : "99%";
    ...
    *** @param oindentLevel The indent level
    *** @param reqState     The current session state containing the EventData selection criteria
    *** @param statusCodes  The status-codes to which the map event data will be limited
    **/

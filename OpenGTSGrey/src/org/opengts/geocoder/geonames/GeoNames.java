    ...
    implements ReverseGeocodeProvider, SubdivisionProvider, GeocodeProvider
{
    ...
    protected static final String PROP_postalFailover           = "postalFailover";         // Boolean: true
    ...
    public static GeoPoint getPostalCodeLocation(String postalCode, String country, int timeoutMS)
    {
        ...
        if (geonames.getTagName().equalsIgnoreCase(TAG_geonames)) {
            NodeList codeList = geonames.getElementsByTagName(TAG_code);
    ...
    **/
    ...
    {
        ReverseGeocode rg = this.getAddressReverseGeocode(gp);
        if (rg == null) {
            // standard RG failed, try postal-code lookup?
            RTProperties rtProp = this.getProperties();
            String postalFailover = rtProp.getString(PROP_postalFailover,"default");
            if (StringTools.isBoolean(postalFailover,true)) {
                // "postalFailover" explicity set true/false
                if (StringTools.parseBoolean(postalFailover,false)) {
                    // "postalFailover" is explicitly "true"
                    rg = this.getPostalReverseGeocode(gp);
                }
            } else {
                // "postalFailover" not specified (or specified "default")
                if (!this.hasUsername()) {
                    // no username likely indicates the 'free' service, try postal lookup
                    // rg = this.getPostalReverseGeocode(gp);
                }
            }
        }
    ...
    public GeoPoint getGeocode(String address, String country)
    {
        ...
        if ((codeList == null) || (codeList.getLength() == 0)) {
            codeList = geonames.getElementsByTagName(TAG_geoname);
            if ((codeList == null) || (codeList.getLength() == 0)) {
    ...
    **/
    protected StringBuffer getGeonamesURL(boolean primary, String service, GeoPoint gp)
    {
        ...
        if (!StringTools.isBlank(host)) {
            ...
            // URL
            sb.append("http://").append(host).append("/").append(service).append("?");
            // optional username
    ...
    **/
    public ReverseGeocode getPlaceNameReverseGeocode(GeoPoint gp)
    {
        ...
        if (geonames.getTagName().equalsIgnoreCase(TAG_geonames)) {
            ...
            /* parse addresses */
            NodeList placeList = geonames.getElementsByTagName(TAG_geoname);
    ...
    **/
    private ReverseGeocode createPlaceName(Map<String,String> placeProps)
    {
        ...
        boolean isUS = (countryCode != null) && countryCode.equals(ReverseGeocode.COUNTRY_US);
        if (isUS) {
    ...
    **/
    public ReverseGeocode getAddressReverseGeocode(GeoPoint gp)
    {
        ...
        if (geonames.getTagName().equalsIgnoreCase(TAG_geonames)) {
            ...
            NodeList addressList = geonames.getElementsByTagName(TAG_address);
    ...
    **/
    private ReverseGeocode createAddress(Map<String,String> addrProps, double maxAddressDistanceKM)
    {
        ...
        boolean isUS = (country != null) && country.equals(ReverseGeocode.COUNTRY_US);
    ...
    **/
    ...
    private String appendPostalCodeElement(StringBuffer sb, Map codeProps, String key, int maxLen, boolean suffixComma)
    {
        ...
        if (!StringTools.isBlank(elem)) {
            if ((maxLen > 0) && (elem.length() > maxLen)) {
                int p = elem.indexOf("("); //
    ...
    **/
    ...
    public String getSubdivision(GeoPoint gp)
    {
        ...
        if (geonames.getTagName().equalsIgnoreCase(TAG_geonames)) {
            ...
            NodeList subdivList = geonames.getElementsByTagName(subDivTAG);
            if (subdivList != null) {
    ...
    **/
    public static void main(String argv[])
    {
        ...
        /* Reverse Geocoding */
        Print.sysPrintln("RevGeocode = " + gn.getReverseGeocode(gp));

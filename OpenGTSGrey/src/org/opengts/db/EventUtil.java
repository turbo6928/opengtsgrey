...
public class EventUtil
{
    ...
    **/
    ...
    public boolean writeEvents_CSV(PrintWriter pwout, EventData evdata[], BasicPrivateLabel privLabel)
        throws IOException
    {
        return writeEvents_CSV(pwout, evdata, null, ',', true, privLabel);
    }
    ...
    private String formatHeader_CSV(String f[], char csvSep)
    {
        ...
        for (int i = 0; i < f.length; i++) {
            ...
            if (f[i].equals(EventData.PFLD_deviceDesc)) {
                sb.append("Vehicle");
            } else
  ...
  //public  static final String  CSV_SEPARATOR                  = "|";
    ...
    public String getParseMapEventJS(boolean isFleet, Locale locale, char csvSep)
    {
        ...
        js.append("    this.address   = (fld.length > 14)? fld[14].trim() : '';\n");
      ...
      //js.append("    //if (this.address == '') { this.address = '&nbsp;'; }\n"); // fill with space, so field isn't blank
        js.append("    if (fld.length > 15) {\n");
        js.append("        this.optDesc = new Array();\n");
        js.append("        for (var i = 15; i < fld.length; i++) {\n");
        ...
        throws IOException
    {
        ...
        String dateFmt = (acct != null)? acct.getDateFormat() : BasicPrivateLabel.getDefaultDateFormat();
        String timeFmt = (acct != null)? acct.getTimeFormat() : BasicPrivateLabel.getDefaultTimeFormat();
        if ((acct != null) && (tmz == null)) { tmz = acct.getTimeZone(null); }
        String tmzStr = (tmz != null)? tmz.getID() : null;
        ...
        String   todayTmzFmt = (tmzStr != null)? tmzStr : today.format("z",tmz);
        ...
        if (!isFleet && (latestTime != null)) {
            String lastTmzFmt = (tmzStr != null)? tmzStr : latestTime.format("z",tmz);
            this.write(pwout, PFX2);
        ...
        throws IOException
    {
        ...
        for (int i = 0; i < edp.length; i++) {
            ...
            boolean showFleetIcon = isFleet && !fleetRoute; // && !edp[i].getIsLastEvent();
    ...
    public boolean writeEvents(OutputStream out, 
        EventData evdata[], 
        int format, boolean allTags, BasicPrivateLabel privLabel)
        throws IOException
    {
        PrintWriter pwout = (out != null)? new PrintWriter(out) : null;
        return this.writeEvents(pwout, evdata, format, allTags, privLabel);
    }
        ...
        throws IOException
    {
        if (evdata != null) {
            switch (format) {
                case FORMAT_TXT:
                case FORMAT_CSV:
                    return this.writeEvents_CSV(pwout, evdata, privLabel);
                case FORMAT_KML:
                ...
                default:
                    Print.logError("Unrecognized data format: " + format);
                    return false;
            }
        }
        return false;
    ...
    public static void main(String argv[])
    {
        ...
        try {
            if (devID.equals("*") || devID.equals("ALL")) {
                OrderedSet<String> devIdList = Device.getDeviceIDsForAccount(acctID,null);
                if (devIdList.size() <= 0) {
                    ...
                    EventData.getRangeEvents(
                        ...
                        new DBRecordHandler() {
                            public int handleDBRecord(DBRecord rcd) throws DBException {
                                ...
                                try {
                                    if (ev.updateAddress(false,rhForceUpdate)) {
                                        if (rhUpdate) {

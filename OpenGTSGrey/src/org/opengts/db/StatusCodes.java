...
// Geofence: 0xF200 to 0xF2FF
    ...
    public static final int STATUS_STATE_ENTER          = 0xF2A0;
    // Description:
    //      Device has entered a state

    public static final int STATUS_STATE_EXIT           = 0xF2B0;
    // Description:
...
// ----------------------------------------------------------------------------
    ...
    private static Code _codeArray[] = new Code[] {
        ...
        new Code(STATUS_LOCATION           , "LOCATION"     , I18N.getString(StatusCodes.class,"StatusCodes.location"      ,"Location"      ), true ),
        ...
        new Code(STATUS_STATE_ENTER        , "GEO.ST.ENTR"  , I18N.getString(StatusCodes.class,"StatusCodes.stateEnter"    ,"StateEnter"    )       ),
        new Code(STATUS_STATE_EXIT         , "GEO.ST.EXIT"  , I18N.getString(StatusCodes.class,"StatusCodes.stateExit"     ,"StateExit"     )       ),
        ...
        new Code(STATUS_LOW_BATTERY        , "LOWBATTERY"   , I18N.getString(StatusCodes.class,"StatusCodes.lowBattery"    ,"LowBattery"    ), true ),
    ...
    /* init StatusCode map */
    private static Map<Integer,Code> _initStatusCodeMap(int inclCodes[])
    {
        Map<Integer,Code> scMap  = new OrderedMap<Integer,Code>();

        /* always add STATUS_NONE first */
    ...
    private static Map<Integer,Code> _GetStatusCodeMap()
    {
        if (statusCodeMap == null) {
            ...
            synchronized (_codeArray) {
                if (statusCodeMap == null) { // check again after lock
                    statusCodeMap = _initStatusCodeMap(null);
                }
            }
        }
    ...
    **/
    ...
    {
        synchronized (_codeArray) {
            if (statusCodeMap != null) { Print.logWarn("Re-initializing StatusCode map"); }
            statusCodeMap = _initStatusCodeMap(inclCodes);
        }
    }
    ...
    public static String GetDescription(int code, BasicPrivateLabel pl)
    {
        Code sc = GetCode(code, pl);
        if (sc != null) {
            return sc.getDescription((pl != null)? pl.getLocale() : null);
        } else {

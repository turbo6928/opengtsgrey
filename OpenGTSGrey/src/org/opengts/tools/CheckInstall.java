...
public class CheckInstall
{
    ...
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(DBConfig.SERVICE_ACCOUNT_ID)) {
            String srvID   = RTConfig.getString(DBConfig.SERVICE_ACCOUNT_ID  ,"?");
            String srvName = RTConfig.getString(DBConfig.SERVICE_ACCOUNT_NAME,"?");
        ...
        // StartupInit class
        {
            String startupInitClass = RTConfig.getString(DBConfig.STARTUP_INIT_CLASS,"");
            String waStartupInitClass = (webappProps != null)? webappProps.getString(DBConfig.STARTUP_INIT_CLASS,"") : "";
            if (StringTools.isBlank(startupInitClass) && StringTools.isBlank(waStartupInitClass)) {
                printVariable(DBConfig.STARTUP_INIT_CLASS, "(default)", "");
            } else {
                String initClass = !StringTools.isBlank(startupInitClass)? startupInitClass : waStartupInitClass;
                printVariable(DBConfig.STARTUP_INIT_CLASS, ClassName(initClass), "");
                ...
                if (!startupInitClass.equals(waStartupInitClass)) {
                    ...
                    addError("webapp.conf '"+DBConfig.STARTUP_INIT_CLASS+"' does not match default.conf",
                             null,
        ...
        // RuleFactory
        {
            ...
            if (ruleFact != null) {
                String v = "[" + ruleFact.getName() + " " + ruleFact.getVersion() + "] " + ClassName(ruleFact);
                printVariable("(RuleFactory)", v, "");
            } else {
        ...
        println("private.xml:");
        {
            ...
            if (BasicPrivateLabelLoader.hasParsingErrors()) {
                ...
                if (defaultPrivLabel != null) {
                    ...
                    int WC = countWarning("'private.xml' does not define a default 'Domain'");
                    println(PFX+"WARNING["+WC+"]: 'private.xml' does not define a default 'Domain'.");
                }
                // populate a set of BasicPrivateLabel's to test
                String privLabelNames[] = BasicPrivateLabelLoader.getPrivateLabelNames();
                for (int i = 0; i < privLabelNames.length; i++) {
                    BasicPrivateLabel privLabel = BasicPrivateLabelLoader.getPrivateLabel(privLabelNames[i]);
                    if (privLabel != null) {
                        if (!privLabelSet.contains(privLabel)) {
                            privLabelSet.add(privLabel);
                        }
                    } else {
                        println(PFX+"ERROR: Unexpected error PrivateLabelName not found: " + privLabelNames[i]);
                        addError("Unexpected error PrivateLabelName not found: " + privLabelNames[i],
                ...
                for (BasicPrivateLabel privLabel : privLabelSet) {
                    ...
                    boolean isDefault = alias.equals("default");
                    ...
                    StringBuffer aliasInfo = new StringBuffer();
                    aliasInfo.append(privLabel.getLocale().toString());
                    if (privLabel.getAccountLogin())    { aliasInfo.append(", accountLogin"); }
                    if (privLabel.getUserLogin())       { aliasInfo.append(", userLogin");  }
                    if (privLabel.getAllowEmailLogin()) { aliasInfo.append(", emailLogin"); }
                    if (privLabel.getEnableDemo())      { aliasInfo.append(", demo");       }
                    if (privLabel.isRestricted())       { aliasInfo.append(", restricted"); }
                    printVariable((domainNdx++) + ") "+alias, aliasInfo.toString(), "");
                    ...
                    if (!skipDefaultEMailChecks) {
                        ...
                        for (int e = 0; e < email.length; e++) {
                            if (email[e].endsWith(SendMail.EXAMPLE_DOT_COM)) {
                                if (hasSMTPHost) {
                                    ...
                                    if (emailErrors == 0) {
                                        addError("EMail addresses for Domain '"+alias+"' have not been customized.",
                                                 null,
                        ...
                        if (privLabel.getBooleanProperty(PrivateLabel.PROP_ReportMenu_enableReportEmail,true)) {
                            ...
                            if (StringTools.isBlank(frEmail)) {
                                ...
                                //         null,
                                //         "Configure outbound SMTP, then re-run this installation check.");
                                int WC = countWarning("Report email defined, but not 'From' address configured");
                    ...
                    for (int a = 0; a < aclList.length; a++) {
                        if (!privLabel.hasAclEntry(aclList[a])) {
                            ...
                            if (aclErrors++ == 0) {
                                addError("ACL list for Domain '"+alias+"' is missing a reserved entry.",
                                         null,
                    ...
                    */
                    ...
                    try {
                        if (privLabel instanceof PrivateLabel) {
                            ...
                            if (mp == null) {
                                if (BasicPrivateLabelLoader.isTrackServlet()) {
                                    println(PFX+"ERROR: No active MapProvider defined ["+alias+"]");
                                    addError("Domain '"+alias+"' is missing an active MapProvider declaration.",
                                             null,
                                             "Add a MapProvider declaration to this Domain, then re-run this installation check");
                                } else {
                                    int WC = countWarning("Make sure Domain '"+alias+"' has an active MapProvider");
                                ...
                                if (!skipDefaultMapChecks && (mp instanceof MapProviderAdapter) && !((MapProviderAdapter)mp).validate()) {
                                    ...
                                    addError("MapProvider '"+mpName+"' returned a validation error for Domain '"+alias+"'",
                                             null,
                                             FIX_PREVIOUS_ERRORS);
                                }
                            }
            ...
            if (dcServerList.isEmpty()) {
                ...
                for (DCServerConfig dcs : dcServerList) {
                    ...
                    if (dcs.serverJarExists()) {
                        printVariable(" " + name, dcs.toString(false), "");
                    } else
                    if (dcUndefSet.contains(name)) {
                        printVariable(" " + name, dcs.toString(false), "");
                        int WC = countWarning("Reference server does not exist: " + name);
                        println(PFX+"WARNING["+WC+"]: Server referenced in runtime config not found: "+name);
                    } else {
                        // ignore these
                    }
                }

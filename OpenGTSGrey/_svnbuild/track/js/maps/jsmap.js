...
**/
function JSDetailPoint(rcdNdx, dsNdx, ppNdx, evRcd)
{
...
function jsmParseXMLPoints(xmlText, recenterMode, replay) // tmz
{
    ...
    for (var dsi = 0; dsi < dataSets.length; dsi++) {
        ...
        var routeColor  = getXMLNodeAttribute(dsAttr, ATTR_color, ROUTE_LINE_COLOR); // "#FF2222");
        ...
        for (var p = startNdx; p < pts.length; p++) {
            ...
            if (isPOI) {
                if (evRcd.validGPS) {
                    rcdNdx++;
                    poiPinList.push(jsmCreatePushPin(rcdNdx, dsi, poiPinList.length, evRcd));
                }
                continue;
            }
            ...
            if (evRcd.validGPS) {
                ...
                pushPinList.push(jsmCreatePushPin(rcdNdx, dsi, ppNdx, evRcd));
                detailList.push(new JSDetailPoint(rcdNdx, dsi, ppNdx, evRcd));
            } else {
                detailList.push(new JSDetailPoint(rcdNdx,  -1,    -1, evRcd));
            }
...
function jsmParseGeozones(zonePoints)
{
    ...
    //   3 - ZONE_POLYGON
    jsmap.JSDrawGeozone(jsvZoneType, jsvZoneRadiusMeters, zonePoints, zoneIndex);
...
function jsmShowDetailPushpin(dsNdx, ppNdx)
{
    ...
    /* no datasets? */
    if ((jsvDataSets == null) || (jsvDataSets.length <= 0) || (dsNdx >= jsvDataSets.length)) {
        // skip info box/bubble
        return;
    }
...
*** @param fill The fill-collr
*** @param e    The 'MapEventRecord' object
**/
function evLabelIconURL(icon, fr, fill, e)
{
    var text = e.devVIN;
...
*** @param e  The 'MapEventRecord' object
**/
function evTextLabelIconURL(icon, fr, fill, border, text)
{

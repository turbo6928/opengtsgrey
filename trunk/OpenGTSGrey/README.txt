...
2) When updating to a newer version of OpenGTS, it is recommended that the following
   command be run to update the database with the latest table/column changes:
     > bin/dbAdmin.pl -tables=ca
3) This verison of OpenGTS now requires Java JDK 6.
...
5) Documentation is now available online at the following link:
   ...
   Available documentation currently includes the following:
     - OpenGTS Installation/Configuration Manual.
     - Tutorial and User's Guide for OpenGTS.
     - Guide to Integrating OpenGTS into Eclipse (by Pierluigi Bucolo).
...
Visit the following link for additional downloadable add-ons for OpenGTS:
...
Contributors:
  See the following link for a list of contributors:
    http://www.opengts.org/contact.html

...
public abstract class DBRecordKey<gDBR extends DBRecord>
{
    ...
    **/
    ...
    {
        try {
            return this._exists(null, true); // full primary key lookup
    ...
    **/
    ...
    {
        try {
            return this._exists(null, fullKeyOnly); // primary key lookup
        } catch (SQLException sqe) {
            String tn = this.getTableName();
            throw new DBException("Record existance " + tn + "='" + this + "'", sqe);
        }
    ...
    **/
        ...
        throws DBException
    {
        try {
            ...
            return this._exists(indexName, true);  // alternate key lookup
        } catch (SQLException sqe) {
            String tn = this.getTableName();
            throw new DBException("Record existance " + tn + "='" + this + "'", sqe);
        }
    ...
    *** @param fullKeyOnly    True to require a full key match, false to allow a partial key match
    *** @return True if the specified key attribute exists in the table, false otherwise
    **/
    protected boolean _exists(String altIndexName, boolean fullKeyOnly)
        throws SQLException, DBException
    {
        ...
        try {
            stmt = DBConnection.getDefaultConnection().execute(dsel.toString()); // may throw DBException
            ...
            if (sqe.getErrorCode() == DBFactory.SQLERR_TABLE_NOTLOCKED) {
                ...
                if (DBProvider.lockTableForRead(this.getTableName(),true)) { // may throw DBException
                    stmt = DBConnection.getDefaultConnection().execute(dsel.toString()); // may throw SQLException, DBException
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* Record */
        NodeList rcdList = gtsResponse.getElementsByTagName(TAG_Record);

...
directory location "/opt/OpenGTS_1.2.3".  Please adjust this name accordingly to 
match your particular OpenGTS installation directory.
...
the gc101 server as follows:
   /zzz> cd /opt/OpenGTS_1.2.3
   /opt/OpenGTS_1.2.3> ant gc101
...
the build directory "/opt/OpenGTS_1.2.3/build/gc101".  The target "gc101.war" then
creates the 'web archive' file "/opt/OpenGTS_1.2.3/build/gc101.war".

If you wish, you can also make temporary changes to the "webapp.conf" file in the 
build directory "/opt/OpenGTS_1.2.3/build/gc101/WEB-INF/", then re-create the 
...
To create the device record enter the following command:
   /opt/OpenGTS_1.2.3> bin/admin.pl Device -account=myaccount -device=gc101 -create

Edit the new record to set the UniqueID for the GC-101 device:
   /opt/OpenGTS_1.2.3> bin/admin.pl Device -account=myaccount -device=gc101 -edit
...
You can monitor the Apache Tomcat log files at "$CATALINA_HOME/logs/catalina.out" to
see the incoming connections from the GC-101 device.  If all has been configured
properly, the GC-101 should send its GPS information on the scheduled periodic basis
to your server, which will install the data into your EventData table (assuming that
the database and "webapps.conf" have been properly configured and initialized.

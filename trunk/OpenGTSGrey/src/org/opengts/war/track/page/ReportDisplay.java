    ...
    implements Constants
{
    ...
    /* DEBUG: True to always expands CSS file contents in the HTML output (should be false) */
    private static final boolean ALWAYS_EXPAND_CSS_FILES    = false;
        ...
        throws ReportException, IOException
    {
        ...
        /* write frame */
        CommonServlet.setResponseContentType(response, HTMLTools.CONTENT_TYPE_HTML);
        ...
        throws IOException
    {
      ...
      //String rptMenuURL = EncodeMakeURL(reqState, Track.BASE_URI(), rptMenu);
        ...
        if (rptFormat.equalsIgnoreCase(ReportPresentation.FORMAT_HTML)) {
            ...
            if (refreshURL != null) {
                report.setRefreshURL(refreshURL);
            }
        ...
        if (rptFormat.equalsIgnoreCase(ReportPresentation.FORMAT_XML)) {
            try {
                ...
                // (See "org.opengts.war.report.ReportBody:writeXML")
        ...
        if (rptFormat.equalsIgnoreCase(ReportPresentation.FORMAT_CSV)) {
            try {
                ...
                // (See "org.opengts.war.report.ReportBody:writeCSV")
                int count = report.writeReport(ReportPresentation.FORMAT_CSV, response.getWriter());
                if (count > 0) {
                    // all is ok
                    return;
                } else {
        ...
        /* EMail report */
        if (ALWAYS_EXPAND_CSS_FILES || rptFormat.equalsIgnoreCase(ReportPresentation.FORMAT_EHTML)) {
            // set Report JSP

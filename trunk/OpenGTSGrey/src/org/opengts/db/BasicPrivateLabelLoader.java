...
public class BasicPrivateLabelLoader
{
    ...
    public  static final String TAG_AltIcon                 = "AltIcon";
    ...
    public  static final String ATTR_userLogin              = "userLogin";              // false
    ...
    public  static final String ATTR_altIcon                = "altIcon";
    ...
    **/
    ...
    protected static boolean    isTrackServlet = false;

    public static void setTrackServlet_debugOnly()
    {
        BasicPrivateLabelLoader.isTrackServlet = true;
    }
    
    public static boolean isTrackServlet()
    {
        return BasicPrivateLabelLoader.isTrackServlet;
    }
        ...
        if (BasicPrivateLabelLoader.privateLabelLoader == null) {
            ...
            if (!BasicPrivateLabelLoader.isTrackServlet) {
                if (!RTConfig.isWebApp()) {
                    ...
                    if (!BasicPrivateLabelLoader.isTrackServlet) {
                        printWarn("Not a 'Track/Service' servlet (some sections may be omitted).");
                    }
                }
            }

            /* attempt to use PrivateLabelLoader for the 'Track'/'Service' servlet */
            if (ALWAYS_LOAD_WAR_PRIVATELABEL || isTrackServlet()) {
                try {
                    ...
                    if (isTrackServlet()) {
                        ...
                        Print.logInfo("Loading default BasicPrivateLabelLoader");
                    }
    ...
    **/
    protected int _resetLoadXML(File xmlFile)
    {
        ...
        int count = this._loadXML(xmlFile, null, null);
        if (count == 0) {
    ...
    *** @return The number of domains loaded
    **/
    protected int _loadXML(File xmlFile, RTProperties dftProps, RTProperties overrideProps)
    {
        ...
        /* Properties: parse default <Properties> (if present) */
        RTProperties defaultProps = new RTProperties(dftProps);
        NodeList propsNodes = privLabels.getElementsByTagName(TAG_Properties);
        for (int p = 0; p < propsNodes.getLength(); p++) {
            Element props = (Element)propsNodes.item(p);
            this.parseTag_Properties(xmlFile, defaultProps, props);
        }
        ...
        NodeList domainList = privLabels.getElementsByTagName(TAG_Domain);
        for (int d = 0; d < domainList.getLength(); d++) {
            Element domain = (Element)domainList.item(d);

            /* parse 'Domain' tag */
            this.parseTag_Domain(xmlFile, i18nPkgName, domain, defaultProps, timeZones);
        ...
        NodeList includeList = privLabels.getElementsByTagName(TAG_Include);
        if ((includeList.getLength() > 0) && (DomainIncludeRecursionLevel > 0)) {
            ...
            for (int i = 0; i < includeList.getLength(); i++) {
                ...
                if ((inclFile != null) && inclFile.isFile()) {
                    if (inclProps != null) {
                        RTConfig.pushTemporaryProperties(inclProps);
                    }
                    try {
                        ...
                        if (inclFilePath.equals(xmlFile.getCanonicalPath())) {
                            ...
                            int cnt = this._loadXML(inclFile, defaultProps, null); // recursive load
                            count += cnt;
                        }
                    } catch (Throwable th) {
                    ...
                    if (inclProps != null) {
                        RTConfig.popTemporaryProperties(inclProps); // inclProps
                    }
                } else {
    ...
    /**
    *** Parse 'Timezones' tag
    *** @param elemNode  The 'Timezones' node
    ...
    **/
    protected OrderedSet<String> parseTag_Timezones(Element elemNode)
    {
        ...
        NodeList timeZonesNodes = elemNode.getElementsByTagName(TAG_TimeZones);
    ...
    **/
        ...
        OrderedSet<String> timeZones)
    {
        ...
        String  locale       = XMLTools.getAttribute(       domain, ATTR_locale      , null , true );
        ...
        i18nStr.setString("Domain.locale",locale);
        ...
        pl.setRTProperties(defaultProps);
        pl.setAliasName(domainName);     // "name" alias for host
        ...
        pl.setLocaleString(locale);
        pl.setTimeZones(domainTMZ);
        //printDebug("Loading 'Domain': " + pl.getAliasName());

        /* push temporary properties */
        // This makes all PrivateLabel properties available within subsequent RTConfig property calls
        pl.pushRTProperties();
        ...
        for (int c = 0; c < attrList.getLength(); c++) {
            ...
            //    printWarn("Found deprecated '"+TAG_Property+"' tag");
            //    this.parseTag_Property(xmlFile, pl, attrElem, null);
            //} else
            ...
            if (attrName.equalsIgnoreCase(TAG_MapProvider)) {
                if (isTrackServlet()) {
                    this.parseTag_MapProvider(xmlFile, i18nPkgName, pl, attrElem);
                }
            } else
        ...
        /* display PrivateLabel properties */
        //pl.printProperties();

        /* save this PrivateLabel */
        this._addPrivateLabel(xmlFile, pl);
    ...
    **/
    ...
    {
        String alias = StringTools.trim(XMLTools.getAttribute(attrElem,ATTR_host,"",true)).toLowerCase();
        if (StringTools.isBlank(alias)) {
            alias = XMLTools.getNodeText(attrElem,"",true).toLowerCase();
        }
        if (this.isValidHostname(alias)) {
            pl.addHostAlias(alias);
        } else {
    ...
    **/
    ...
    {
        String  keyPrefix = XMLTools.getAttribute(attrElem,ATTR_rtPropPrefix,null,true);
        NodeList propList = attrElem.getChildNodes();
        for (int e = 0; e < propList.getLength(); e++) {
    ...
    **/
    protected void parseTag_Property(File xmlFile, RTConfig.PropertySetter ps, Element attrElem, String keyPrefix)
    {
        ...
        //Print.logInfo("Property key = %s/%s", (keyPrefix + rtKey), key);
        if (!StringTools.isBlank(key)) {
    ...
    **/
    private AccessLevel[] _parseAccessLevelValues(String strList)
    {
        ...
        if (v.length == 1) {
            return AclEntry.GetValueListForMaximumAccessLevel(this._parseAccessLevel(v[0],AccessLevel.ALL));
        } else {
            AccessLevel lastValue = null;
            java.util.List<AccessLevel> values = new Vector<AccessLevel>();
            for (int i = 0; i < v.length; i++) {
                AccessLevel acc = this._parseAccessLevel(v[i], AccessLevel.ALL);
    ...
    /**
    *** return the AccessLevel for the specified argument
    **/
    private AccessLevel _parseAccessLevel(String accStr, AccessLevel dft)
    {
        int accInt = StringTools.parseInt(accStr, (dft != null)? dft.getIntValue() : 1);
    ...
    **/
    protected void parseTag_Acls(File xmlFile, String i18nPkgName, BasicPrivateLabel pl, Element acls)
    {
        ...
        /* default access */
        AccessLevel dftAccess = this._parseAccessLevel(XMLTools.getAttribute(acls,ATTR_default,null,false), null); 

        /* parse ACL entries */
        Map<String,AclEntry> aclMap = new OrderedMap<String,AclEntry>();
        NodeList aclList = acls.getElementsByTagName(TAG_Acl);
        for (int r = 0; r < aclList.getLength(); r++) {
            ...
            if (!StringTools.isBlank(aclName)) {
                ...
                AccessLevel valAcc[] = this._parseAccessLevelValues(accValues);
                AccessLevel dftAcc   = this._parseAccessLevel(XMLTools.getAttribute(aclElem,ATTR_default,null,false), dftAccess);
    ...
    *** @return The BasicPrivateLabel instance
    **/
    public static BasicPrivateLabel getPrivateLabel(URL url)
    {
        BasicPrivateLabelLoader bpll = BasicPrivateLabelLoader._getInstance();
        if (url != null) {
            String host = (url != null)? StringTools.trim(url.getHost()) : null;
            ...
            return bpll._getPrivateLabel(host); // will return default if still not found
        } else {
            return bpll._getPrivateLabel(null); // return default
        }
    }
    ...
    **/
    public static String[] getPrivateLabelNames()
    {
    ...
    *** @return A String array of all private label keys/names
    **/
    public static String[] getPrivateLabelNames(boolean nameOnly)
    {
        ...
        if (privLblMap != null) {
            if (nameOnly) {
                for (Iterator<String> i = privLblMap.keySet().iterator(); i.hasNext();) {
                    ...
                    String name = pbl.getAliasName();
                    if (StringTools.isBlank(name)) { name = pbl.getHostName(); }
    ...
    **/
    ...
    /**
    *** Called by this BasicPrivateLabelLoader instance (or subclass) is a a parsing error was encountered
    **/
    ...
    *** @return The BasicPrivateLabel instance
    **/
    private BasicPrivateLabel _getPrivateLabel(String hostName, BasicPrivateLabel dftPrivLabel) 
    {
        // get custom private label based on domain name 
        if ((hostName != null) && (this.privateLabelMap != null)) {
            String host = StringTools.trim(hostName);
            BasicPrivateLabel plbl = this.privateLabelMap.get(host);
    ...
    **/
    protected void _addPrivateLabel(File xmlFile, BasicPrivateLabel privLabel)
    {
        if (privLabel != null) {
            ...
            if (StringTools.isBlank(host) || host.equalsIgnoreCase(BasicPrivateLabel.DEFAULT_HOST)) {
                if (this.defaultPrivateLabel == null) {
                    host = BasicPrivateLabel.DEFAULT_HOST;
                    privLabel.setHostName(host);
                    this.defaultPrivateLabel = privLabel;
                    this.privateLabelMap.put(host, privLabel);
                } else {
            ...
            if (!this.privateLabelMap.containsKey(host)) {
                this.privateLabelMap.put(host, privLabel);
            } else {
                // alias already defined
                printError("Domain Host/Alias already defined: [" + xmlFile + ":" + privLabel.getName() + "] " + host);
                this._setHasParsingErrors(xmlFile);
            }

            /* add BasicPrivateLabel under 'name' alias */
            String alias = privLabel.getAliasName();
            if (!StringTools.isBlank(alias)) {
                if (!this.privateLabelMap.containsKey(alias)) {
                    this.privateLabelMap.put(alias, privLabel);
                } else {
                    // alias already defined
                    printError("Domain Host/Alias already defined: [" + xmlFile + ":" + privLabel.getName() + "] " + alias);
                    this._setHasParsingErrors(xmlFile);
                }
            }

            /* add BasicPrivateLabel under host 'domain' aliases */
            java.util.List hostAliasList = privLabel.getHostAliasList();
            if (!ListTools.isEmpty(hostAliasList)) {
                for (Iterator i = hostAliasList.iterator(); i.hasNext();) {
                    String hostAlias = (String)i.next();
                    if (!StringTools.isBlank(hostAlias)) {
                        if (!this.privateLabelMap.containsKey(hostAlias)) {
                            this.privateLabelMap.put(hostAlias, privLabel);
                        } else {
                            // alias already defined
                            printError("Domain Host/Alias already defined: " + hostAlias);
                            this._setHasParsingErrors(xmlFile);
                        }
                    }
                }

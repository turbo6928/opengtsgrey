...
if [ "$CATALINA_HOME" != "" ]; then
    if   [ -f "${CATALINA_HOME}/common/lib/servlet-api.jar" ]; then
        CPATH="${CPATH}${PATHSEP}${CATALINA_HOME}/common/lib/servlet-api.jar"
       #CPATH="${CPATH}${PATHSEP}${CATALINA_HOME}/common/lib/naming-factory-dbcp.jar"
    elif [ -f "${CATALINA_HOME}/lib/servlet-api.jar" ]; then
        CPATH="${CPATH}${PATHSEP}${CATALINA_HOME}/lib/servlet-api.jar"
    fi
fi
...
COMMAND="${CMD_JAVA} -Djava.awt.headless=true -classpath ${CPATH} ${JMAIN} '-conf=${GTS_CONF}' ${DEBUG_ARGS} ${ARGS} $1 $2 $3 $4"
if [ ${GTS_DEBUG} -ne 0 ]; then
    echo "${COMMAND}"
fi
